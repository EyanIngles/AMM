...
export const loadTokens = async (provider, chainId, dispatch) => {
    const dapp = await new ethers.Contract(config[chainId].dapp.address, TOKEN_ABI, provider)
    const ease = await new ethers.Contract(config[chainId].ease.address, TOKEN_ABI, provider)
