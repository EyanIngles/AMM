...
const Deposit = () => {
  ...
  const amountHandler = async (e) => {
    if (e.target.id === 'token1') {
        ...
        const _token1Amount = (e.target.value)
        const result = await amm.calculateToken1Deposit(_token1Amount)
        const _token2Amount = (result.toString())
        console.log(await amm.calculateToken1Deposit(_token1Amount))
        setToken2Amount(_token2Amount)
    } else {
        setToken2Amount(e.target.value)

        const _token2Amount = (e.target.value)
        const result = await amm.calculateToken1Deposit(_token2Amount)
        const _token1Amount = (result.toString())
