...
describe('AMM', () => {
  ...
  describe('Swapping tokens', () => {
    ...
    it('facilitates swaps', async () => {
      ...
      // deployer approves all tokesn
      transaction = await token2.connect(deployer).approve(amm.address, tokens(1))
      await transaction.wait()

      // Check deployer balance before swap
      balance = await token1.balanceOf(deployer.address)
      console.log(`deployer Token1 balance before swap: ${ethers.utils.formatEther(balance)}`)

      // Estimate amount of tokens deployer will receive after swapping token2: includes slippage
      estimate = await amm.calculateToken2Swap(tokens(1))
      console.log(`Token1 Amount deployer will receive after swap: ${ethers.utils.formatEther(estimate)}`)

      // deployer swaps 1 token
      transaction = await amm.connect(deployer).swapToken2(tokens(1))
      await transaction.wait()

      // Investor2 approves all tokesn
      transaction = await token2.connect(investor2).approve(amm.address, tokens(1))
      await transaction.wait()
   ...
   // it("successfully calls upon swap token1 function", async () => {
   //     let transaction = await token1.connect(deployer).approve(amm.address, tokens(100))
   //     let result = await transaction.wait()
//
   //     console.log(result, "result of approve function")
   //     transaction = await amm.connect(deployer).swapToken2(tokens(100))
   //     result = await transaction.wait()
   //     console.log(result)
//
   // })
