...
describe('AMM', () => {
    ...
    describe('Swapping tokens', () => {
        it('filicates swaps', async () => {
            ...
            estimate = await amm.calculateToken1Swap(tokens(10))
            console.log(`token2 amount investor1 will receive after swap: ${ethers.utils.formatEther(estimate)}`)

            transaction = await amm.connect(investor1).token1Swap(tokens(10))
            result = await transaction.wait()

            console.log(`token2 amount investor1 will receive after swap: ${ethers.utils.formatEther(result)}`)
