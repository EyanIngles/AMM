...
describe('AMM', () => {
    ...
    describe('Swapping tokens', () => {
        it('filicates swaps', async () => {
            ...
            transaction = expect(await token1.balanceOf(amm.address)).to.equal(amount)
            transaction = expect(await token2.balanceOf(amm.address)).to.equal(amount)

            transaction = expect(await amm.token1Balance()).to.equal(amount)
            transaction = expect(await amm.token2Balance()).to.equal(amount)
            await transaction.wait()


            console.log(await amm.K().toString())
        })
    })
})