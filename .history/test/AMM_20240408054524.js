...
describe('AMM', () => {
    ...
    describe('Swapping tokens', () => {
        it('filicates swaps', async () => {
            ...
            transaction = await token1.connect(deployer).approve(amm.address, tokens(50000))
            await transaction.wait()
            transaction = await token2.connect(deployer).approve(amm.address, tokens(50000))
            await transaction.wait()

            //LP adds liquidity for approved amount.
            transaction = await amm.connect(liquidityProvider).addLiquidity(tokens(50000), tokens(50000))
            await transaction.wait()

            expect(await amm.shares(liquidityProvider.address)).to.equal((50))
        })
    })
})